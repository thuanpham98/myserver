...
async function postConfigEquipi_Port(){
    ...
    for(let i = 0;i<num_equipi;i++){
        let id = 'po' + i.toString();
        temp = document.getElementById(id).value;
        temp=temp.toString();
        console.log(temp);
        if((temp!=="") && !(isNaN(temp))){
            sub_frame.push({index : i, port :parseInt(temp,10)});
        }
    }
...
document.getElementById("updatePort").addEventListener("click", function(){
    postConfigEquipi_Port();
    console.log("press");
});